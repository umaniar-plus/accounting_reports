...
export class AgedPayableReport extends Component {
    ...
    groupDataByPartner() {
        ...
        sortedLines.forEach(line => {
            ...
            const asOfDate = new Date(this.state.asOfDate);
                    
            const agingDate =
                this.state.agingBasis === 'invoice_date'
                    ? new Date(line.date)
                    : new Date(line.date_maturity || line.date);
                    
            const ageDays = Math.floor(
                (asOfDate - agingDate) / (1000 * 60 * 60 * 24)
            );
            ...
            const amount = Math.abs(line.amount_residual !== undefined ? line.amount_residual : line.balance || 0);
