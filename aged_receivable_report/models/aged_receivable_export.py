...
class agedReceivableExport(models.TransientModel):
    _name = 'aged.receivable.export'
    _description = 'aged receivable Export'
    ...
    def _generate_ap_filename(self, data, file_ext="pdf"):
        ...
        return f"aged_receivable_{filename_core}.{file_ext}"
    ...
    @api.model
    def export_aged_receivable_pdf(self, data):
        ...
        """Export aged receivable to PDF using Odoo QWeb template"""
        ...
        if 'column_config' not in data:
            config = self.env['aged.receivable.report.config'].get_active_config()
        ...
        try:
            report_ref = 'aged_receivable_report.aged_receivable_pdf_export'
            ...
            try:
                ...
                return {
                    ...
                    'report_name': 'aged_receivable_report.aged_receivable_pdf_export',
                    'data': {'data': data},
                    'report_type': 'qweb-pdf'
                }

    @api.model
    def export_aged_receivable_xlsx(self, data):
        data['format_currency'] = self.format_currency_helper
        collapsed_partners = data.get('collapsed_partners', {})
        try:
            output = io.BytesIO()
            workbook = xlsxwriter.Workbook(output, {'in_memory': True})
            worksheet = workbook.add_worksheet('Aged Receivable')
            ...
            worksheet.set_column('I:I', 20)  # Balance

            period_text = self.get_period_text(data)

            # Title & filter info
            worksheet.merge_range('A1:I1', 'Aged Receivable Report', title_format)
            ...
            for partner in grouped_data:
                ...
                for line in partner.get('lines', []):
                    ...
                    subtotal_bucket_1 += line.get('bucket_1', 0)

                    subtotal_bucket_2 += line.get('bucket_2', 0)

                    subtotal_bucket_3 += line.get('bucket_3', 0)

                    subtotal_bucket_4 += line.get('bucket_4', 0)

                    subtotal_older += line.get('older', 0)

                    subtotal_total += line.get('total', 0)
            ...
            workbook.close()
            output.seek(0)
            # filename = f"aged_receivable_{data.get('filter_title', 'Report')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
            ...
            _logger.error(f"Error exporting aged receivable to XLSX: {str(e)}")
