...
class PartnerLedgerReportConfig(models.Model):
    _name = 'partner.ledger.report.config'
    _description = 'Partner Ledger Report Configuration'
    _rec_name = "name"
    ...
    show_due_date = fields.Boolean(string="Show Due Date", default=True)
    ...
    _sql_constraints = [
        ('unique_company_config', 'unique(company_id)',
         'Only one partner Ledger Configuration is allowed per company.'),
    ]
